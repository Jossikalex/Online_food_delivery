...
if (current === 'manager-sales.php') {
  Promise.all([
    fetch('manager-sales.php?action=summary').then(r => r.json()),
    fetch('manager-sales.php?action=categories').then(r => r.json())
  ]).then(([sum, cat]) => {
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    form.addEventListener("submit", async (e) => {
        ...
        try {
            const response = await fetch("manager-profile.php", {
                method: "POST",
                body: formData
            });

            // If PHP crashed or returned HTML
            if (!response.ok) {
