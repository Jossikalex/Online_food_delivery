...
$where = '';
if ($search) {
    $where = " WHERE (Fname LIKE '%$search%' OR Lname LIKE '%$search%' OR email LIKE '%$search%' OR phone LIKE '%$search%') ";
}
if ($status !== 'all') {
    $where .= ($where ? ' AND ' : ' WHERE ') . " is_active = " . ($status === 'active' ? 1 : 0);
}
...
$total = (int)(mysqli_fetch_assoc(
    mysqli_query($connect, "SELECT COUNT(*) c FROM Customer $where")
)['c'] ?? 0);
...
if (isset($_POST['action'], $_POST['cust_id'])) {
    ...
    switch ($_POST['action']) {
        case 'toggle':
            mysqli_query($connect, "UPDATE Customer SET is_active = NOT is_active WHERE cust_id = $id");
            break;
        case 'delete':
            mysqli_query($connect, "DELETE FROM Customer WHERE cust_id = $id");
            break;
    }
...
$customers = mysqli_query(
    $connect,
    "SELECT cust_id, CONCAT(Fname,' ',Lname) name, email, phone, is_active, created_at
     FROM Customer $where 
                                    ...
                                    <td>
                                        ...
                                        <form method="POST" style="display:inline;">
                                            ...
                                            <button type="submit" 
                                                ...
                                                <i class="fas fa-<?= $c['is_active'] ? 'lock' : 'unlock' ?>"></i>
                                            </button>
                                        </form>
